...
def create_qr_3(c,data):
    ...
    for i in range(len(data)):
        ...
        c.setFont('Arabic', 8)
        if i%2==0:
            c.drawImage(temp_image_path, 2.3*cm, 22.5*cm-(count*9.9)*cm, width=4*cm, height=4*cm,preserveAspectRatio=True, mask='auto')
            c.drawCentredString(4.3*cm,21.5*cm-(count*9.9*cm),text1)
            c.drawCentredString(4.3*cm,21*cm-(count*9.9*cm),text2)
        else:
            c.drawImage(temp_image_path, 12.8*cm, 22.5*cm-(count*9.9)*cm, width=4*cm, height=4*cm,preserveAspectRatio=True, mask='auto')
            c.drawCentredString(14.8*cm,21.5*cm-(count*9.9*cm),text1)
            c.drawCentredString(14.8*cm,21*cm-(count*9.9*cm),text2)
            count+=1
        ...
        # temp_image_path = tempfile.NamedTemporaryFile(suffix=".png").name
        # img.save(temp_image_path)
        # c.drawImage(temp_image_path, 11.8*cm, 22*cm-(i*9.9)*cm, width=6*cm, height=6*cm,preserveAspectRatio=True, mask='auto')
        # text=arabic_text("للتواصل")
        # c.drawCentredString(14.8*cm,21.5*cm-(i*9.9*cm),text)

def create_pdf_3(self,input_file,output_file,company_name,phone,codes,logo,signature,month1,year):
        """
        ...
        for proc in range (0,len(df)):
            ...
            if (logo!=""):
                #draw logo
                if proc%2==0:
                    c.drawImage(logo, 10.1*cm, 19.5*cm-(count*invoice_width), width=10*cm, height=8*cm,preserveAspectRatio=True, mask='auto')
                else:
                    c.drawImage(logo, -0.6*cm, 19.5*cm-(count*invoice_width), width=10*cm, height=8*cm,preserveAspectRatio=True, mask='auto')
            if(proc_status=="مؤخر"): 
                month=int(month1)
                if(len(str(month))==1):
                    month="0"+str(month)
            ...
            if (proc+1)%6==0 and proc+1!=len(df):   #if it is the last invoice in the page
            ...
            if proc+1==len(df) and (proc+1)%6!=0:   #if it is the last invoice in the page
                c.showPage()    #create new page
                c.setFont('Arabic', 14)
                c.translate(cm,cm) 
                c.setLineWidth(1.5)#width of the line
...
# create_pdf_3('self','العمارات 2.xlsx','out.pdf',"",'01001041961','','final logo.png','sign1.png',
#             1,2024)